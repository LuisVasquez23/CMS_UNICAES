...
const Pagina = ({ page, handleDeletePage }) => {
  ...
  return (
    <Col md={4}>
      <ul className="mt-5">
        <li key={page.id} className="itemCard">
    ...
    async function fetchPages() {
      try {
        const response = await fetch(
          `http://127.0.0.1:8000/api/pages/${userId}`
        );
        if (response.ok) {
  ...
  const handleDeletePage = async (pageId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(
            `http://127.0.0.1:8000/api/pages/${pageId}`,
            {
              method: "DELETE",
            }
          );
    ...
    <>
      <div className="text-center">
        ...
        <Link
          ...
          className="btn btn-success mb-3 align-items-center mt-3"
        >
            ...
            <>
              {pages.length === 0 ? (
                <p className="mt-3 fs-5">
                  No tienes páginas creadas, creemos una!
                </p>
              ) : (
                <Row>
